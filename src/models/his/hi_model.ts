...
export class HisHiModel {
  ...
  getLabs(db: Knex, seq: any) {
    return db(`labresult as r`)
      ...
      .innerJoin(`lablabel as lb`, function () { this.on(`r.labcode`, `lb.labcode`).andOn(`r.lab_code_local`, `lb.fieldname`) })
      .where(`l.vn`, seq)
      .andWhere(`l.finish`, `1`)
      .whereNotIn(`r.labcode`, ['038', '068', '271', '177', '327', '243', '344', '259'])
      ;
  }
